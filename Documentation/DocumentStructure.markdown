...
Если какое-либо поле отсутствует в передаваемого документе, то считается, что его значение равно **null** для строк, массивов и т.п. Для булевых значений это **false**, для числовых **0**.

Черновиком является документ со статусом **draft** или **new**

Поля, описанные как "обязательные" должны присутствовать в документе и иметь непустые значения для объектов, строк и массивов. При отсутствии таких полей документ считается невалидным и не показывается.
...
<code>
	...
	"modified": "20101116T154043Z0000",
	"receivedDate":"20101116T154043Z0000",
	"status":  "draft" ,
	"priority" : 0,
	"author": "Леонов О.В.",
	"editable": true
...
* **modified** - обязательная строковая дата последней модификации документа в формате **yyyy.MM.dd'T'HH:mm:ss'Z'Z**
...
* **editable** - обязательное булевое поле признака возможности редактирования документа и всех связанных с ним объектов
* **receivedDate** - обязательная строковая дата передачи документа помошником в формате **yyyy.MM.dd'T'HH:mm:ss'Z'Z**.
...
<code>
	...
	"document": {
		...
		,"files": [{
					"id": "585907375e42599300f55777baf16c8b",
				  "name": "03_поставка_материального_носителя_с_СМ(Д_2_реквизиты).tiff",
		     "pageCount": 7, 
			 "resources":
			 [ 
				{ 
...
* **performers** - массив строк иденитификаторов исполнителей. Идентификаторы должны быть подмножеством идентификаторов объектов, передаваемых клиенты в виде адресной книги. Порядок следования элементов в массиве имеет значение.
* **deadline** - строковая дата срока исполнения резолюции в формате **yyyyMMdd**
* **hasControl** - булевое значение "на контроле"
* **date** - строковая дата утверждения/отклонения резолюции в формате **yyyy.MM.dd'T'HH:mm:ss'Z'Z**. В черновиках эта дата вычисляется текущей датой в момент открытия документа.
* **parent** - объект вышестоящей резолюции, имеющий следующую структуру:
	* **author** - обязательное строковое значение автора резолюции
	* **date** - обязательная строковая дата вышестоящей резолюции в формате **yyyy.MM.dd'T'HH:mm:ss'Z'Z**
	...
	* **hasControl** - булевое значение "на контроле"	
* **document** - обязательный объект со следующей структурой:
	* **regNumber** - обязательный строковый регистрационный номер резолюции
	...
	* **files** - обязательный непустой массив вложений для резолюции со следующей структурой:
		...
		* **resources** - массив рисований к страницам  со следующей структурой:
			* **pageNum** - обязательный числовой номер страницы, к которой относятся данные ресурсы
			* **drawing** - хеш файла рисования
	* **links** - массив объектов связанных документов, имеющих следующую структуру:
		* **id** - обязательный строковый идентификатор связанного документа. Уникален внутри своего объекта.
...
* **resources** - объект, имеющий два значения:
	* **drawing** - хеш файла рисования
	* **audio** - хеш файла аудио комментария
...
* **date** - строковая дата утверждения/отклонения документа в формате **yyyy.MM.dd'T'HH:mm:ss'Z'Z**. В черновиках эта дата вычисляется текущей датой в момент открытия документа.
* **document** - обязательный объект со следующей структурой:
	* **corrs** - массив строк корреспондентов
...
* **resources** - объект со следующей структурой:
	* **audio** - хеш файла аудио комментария

##Сортировка документов на клиенте в очередях##

* в очереди **необработанные документы** документы должны группироваться по дате **receivedDate** по убыванию. Внутри каждой группы сортировка идет по убыванию важности (поле **priority**) и по убыванию времени **receivedDate**.
...
В информации о документе показывается дата **document.regDate** для резолюций и получения документа с сервера для документов на подпись.