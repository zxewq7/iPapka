...
Обе очереди должны иметь контент в формате стандартного вывода view в формате JSON в Lotus Domino версии 7.0.2 - 8.5.x и иметь обязательную колонку **version**, соответствующей **docVersion** документа
...
* **date** - строковая дата утверждения/отклонения резолюции в формате **yyyyMMddTHHmmss,SS+TZ**. В черновиках эта дата вычисляется текущей датой в момент открытия документа.
...
* **date** - строковая дата утверждения/отклонения документа в формате **yyyyMMddTHHmmss,SS+TZ**. В черновиках эта дата вычисляется текущей датой в момент открытия документа.
* **document** - обязательный объект со следующей структурой:
	* **corrs** - массив строк корреспондентов
...
**protocol**://**hostName**/**databaseName**/document/**docId**/link/**linkId**/file/**fileId**/page/**pageNum**/drawing

URL доступа к аудио комментарию документа:
**protocol**://**hostName**/**databaseName**/document/**docId**/audio
...
* **fileId** - идентификатор файла (вложениея)
...
В информации о документе показывается дата **document.regDate** для резолюций и **created** для документов на подпись.